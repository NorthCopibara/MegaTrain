...
namespace Qbik.Static.Pool 
{
    public class Pool
    {
        ...
        public void NewSinglSpawn(GameObject prefab) 
        {
            int key = prefab.GetInstanceID();
            Stack<GameObject> stack = new Stack<GameObject>();
            cachedObj.Add(key, stack);
        }
        public GameObject SinglSpawn(GameObject prefab) 
        {
            int key = prefab.GetInstanceID();
            GameObject suka = Populate(prefab, Vector3.zero, Quaternion.identity, _parentPool);
            suka.SetActive(false);
            cachedIds.Add(suka.GetInstanceID(), key);
            cachedObj[key].Push(suka);
